...
def entity_kind_for_node(node: dict[str, Any]) -> str:
    explicit = str(node.get("entity_kind") or "").strip()
    if explicit:
        return explicit
    if str(node.get("bib_key") or "").casefold() == "maskaraprivate":
        return "private_communication"
    return "scholarly_work"


def display_title(node: dict[str, Any]) -> tuple[str, bool]:
    raw_title = str(node.get("title") or "").strip()
    if raw_title:
        return raw_title, False

    entity_kind = entity_kind_for_node(node)
    if entity_kind == "private_communication":
        authors = node.get("authors") or []
        bib_key = str(node.get("bib_key") or "")
        identity = (
            (authors[0] if authors else None)
            or ("N. Maskara" if bib_key.casefold() == "maskaraprivate" else None)
            or bib_key
            or node.get("paper_uid")
            or node["id"]
        )
        return f"{identity} · private communication", True

    identifiers = (
        ("DOI", node.get("doi")),
        ("arXiv", node.get("arxiv_id")),
        ("OpenAlex", node.get("openalex_id")),
        ("paper", node.get("paper_uid")),
        ("record", node.get("id")),
    )
    for label, value in identifiers:
        if value:
            return f"Metadata unavailable · {label} {str(value).strip()}", True
    return "Metadata unavailable", True


def detail_file_key(node: dict[str, Any]) -> str:
    paper_uid = str(node.get("paper_uid") or "").strip()
    if paper_uid and all(char.isalnum() or char in "-_" for char in paper_uid):
        return paper_uid
    return hashlib.blake2b(
        str(node["id"]).encode("utf-8"), digest_size=10
    ).hexdigest()
...
def compact_node(
    ...
    *,
    ...
    return {
        ...
        "title": title,
        "titleSource": node.get("title_source"),
    ...
    return {
        "meta": {
            **(graph.get("meta") or {}),
            ...
            "missingTitleCount": sum(
                1 for node in compact_nodes if node["titleMissing"]
            ),
            "yearCounts": sorted(
...
def write_detail_shards(graph: dict[str, Any], output: Path) -> int:
    written = 0
    detail_dir = output / "papers"
...
def main() -> None:
    ...
    core_payload["meta"]["fullNodeCount"] = full_payload["meta"]["nodeCount"]
